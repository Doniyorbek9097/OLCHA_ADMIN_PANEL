  ...
  <q-page class="q-pa-sm">
    ...
    <div class="row" v-else>
      <div class="q-pa-xs col-12">
        <q-card>
          <q-table :filter="search" :grid="grid" flat bordered title="Sub Categories"
            :rows="categoryStore.childCategories" :columns="columns" row-key="index" virtual-scroll
            ...
            <template #top="props">
              <q-toolbar style="padding:0 !important;">
                ...
                <q-btn flat round dense icon="add" :to="{ name: 'AddChildCategory' }" />
              </q-toolbar>
              <q-toolbar>
                <q-space />
            ...
            <template #body-cell-action="props">
              <q-td :props="props">
                <q-btn icon="edit" size="sm" flat dense color="blue"
                  :to="{ name: 'EditChildCategory', params: { id: props.row._id } }" />
                <q-btn icon="delete" size="sm" class="q-ml-sm" flat dense color="red"
                  @click="categoryStore.deleteCategory(props.row._id, categoryStore.childCategories.indexOf(props.row))" />
              </q-td>
            </template>
...
watchEffect(() => $q.screen.lt.sm ? grid.value = true : grid.value = false);
